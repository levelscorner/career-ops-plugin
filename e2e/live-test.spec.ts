 ...
 * - The career-ops extension loaded
...
test.beforeAll(async () => {
  ...
  const tmpProfile = path.join(os.tmpdir(), `career-ops-test-profile-${Date.now()}`);
...
test('LinkedIn: badge appears on a job detail page', async () => {
  ...
  const extErrors = errors.filter((e) => e.includes('career-ops'));
...
test('Naukri: badge appears on a job detail page', async () => {
  ...
  const badgeOnSearch = page.locator('#career-ops-badge-host');
  ...
  const badge = page.locator('#career-ops-badge-host');
  ...
  page.on('console', (msg) => {
    if (msg.text().includes('career-ops')) consoleMessages.push(msg.text());
  });
  await page.waitForTimeout(1000);
